...
def task(num):
    ...
    f = open('C:\\Users\\Mandy\\Desktop\\threadpool\\available' + str(num) + '.csv', 'w', newline='')
    wr = csv.writer(f)
    f2 = open('C:\\Users\\Mandy\\Desktop\\threadpool\\info' + str(num) + '.csv', 'w', newline='')
    wr2 = csv.writer(f2)
    if matched is None:  # 없는페이지 패스
        ...
        url_getkey = 'https://www.myrealtrip.com/offers/' + str(num) + '/options?begin_at=2020-01-17'  # 키 가져올때 참고할url
        ...
        ####################################################
            ...
            for v in d["option_set"]["options"]:
                url_getoptionprice = ('https://www.myrealtrip.com/offers/' + str(num) + '/check_option?choice_'
                                      + 'set[begin_at]=2020-01-17&choice_set[choices][][key]='  #
                                      + str(v["key"]) + '&choice_set[choices][][travelers]=1')  # str(v["key"])가 키값 가져오는거
